...
//! - [`write_text`] writes only the `response` to `out` (plus a clamp warning
//! to `err_out` if applicable) — for `--output-format text` (default).
//! - [`write_json`] serializes **a single buffered JSON object**, with
//! `schema_version` as the **first physical field** (REQ-H14), truncating each large
//! `result`/`content` to the effective cap ([`TOOL_RESULT_CAP`](super::limits::TOOL_RESULT_CAP)
//! by default) with a marker.
//!
...
/// already-redacted string is a no-op. A surviving key-pattern would break this equality (a
/// second pass would redact it), so the assertion is the proxy for "no known key-pattern is
/// ever left un-redacted" (REQ-H15c).
///
