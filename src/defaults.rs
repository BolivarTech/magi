...
/// Anthropic opt-in. Active `[memory]` and `[embedding]` sections are included
/// so the embedding model (the field most likely to cause a 404 when it doesn't
/// match the installed Ollama model) is a live, immediately editable field.
pub fn render_default_magi_toml() -> String {
    format!(
        "# Generated by magi-rs --init-config / /init-config — built-in Ollama-first defaults.\n\
         ...
         # Run: ollama pull {emb_model}\n\
         # The embedding API key is read from OPENAI_API_KEY env only (never in this file).\n\
         [embedding]\n\
         base_url = \"{emb_base}\"   # Ollama default; point elsewhere for cloud\n\
         model    = \"{emb_model}\"  # CHANGE to your installed embedding model — run: ollama pull <model>\n\
        ...
        bal = DEFAULT_MAGI_BALTHASAR,
        cas = DEFAULT_MAGI_CASPAR,
        emb_base = DEFAULT_OPENAI_BASE_URL,
