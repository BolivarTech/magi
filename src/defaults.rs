...
/// Anthropic opt-in. Commented `[memory]` and `[embedding]` stubs are appended
/// so users can discover and uncomment the knobs most likely to need tuning
/// (embedding model, context budget, distillation toggle) without reading docs.
pub fn render_default_magi_toml() -> String {
    format!(
        "# Generated by magi-rs --init-config / /init-config — built-in Ollama-first defaults.\n\
         ...
         # model    = \"{emb_model}\"              # your installed embedding model\n\
