...
impl Tool for ProjectFactTool {
    fn name(&self) -> &str { "project_knowledge" }
    ...
    async fn execute(&self, input: Value) -> ToolResult<Value> {
        let action = input["action"].as_str().ok_or_else(|| ToolError::InvalidArguments("Missing action".to_string()))?;
        
        match action {
            "remember" => {
                let key = input["key"].as_str().ok_or_else(|| ToolError::InvalidArguments("Missing key".to_string()))?;
                let val = input["value"].as_str().ok_or_else(|| ToolError::InvalidArguments("Missing value".to_string()))?;
                
                // MAGI FIX: DoS protection - limit size of stored facts (50KB)
                const MAX_VALUE_SIZE: usize = 50 * 1024;
                if val.len() > MAX_VALUE_SIZE {
                    return Err(ToolError::InvalidArguments(format!("Size limit exceeded: value must be < {} bytes", MAX_VALUE_SIZE)));
                }

                self.memory.set_knowledge(key, val).await
                    .map_err(|e| ToolError::ExecutionError(e.to_string()))?;
                Ok(json!({ "status": "success", "message": format!("Fact '{}' remembered.", key) }))
            }
            "recall" => {
                let key = input["key"].as_str().ok_or_else(|| ToolError::InvalidArguments("Missing key".to_string()))?;
                match self.memory.get_knowledge(key).await.map_err(|e| ToolError::ExecutionError(e.to_string()))? {
                    Some(val) => Ok(json!({ "key": key, "value": val })),
                    None => Ok(json!({ "status": "not_found", "message": format!("No fact found for '{}'", key) })),
                }
            }
            "list" => {
                let keys = self.memory.list_knowledge_keys().await
                    .map_err(|e| ToolError::ExecutionError(e.to_string()))?;
...
mod tests {
    ...
    async fn test_knowledge_tool_execution() {
        ...
        let memory = Arc::new(EncryptedSqliteMemory::new(tmp.path().to_path_buf(), "pass".to_string()).unwrap());
    ...
    async fn test_knowledge_tool_size_limit() {
        ...
        if let Err(crate::tools::ToolError::InvalidArguments(msg)) = res {
            assert!(msg.contains("Size limit exceeded"), "Error message should mention size limit");
        } else {
