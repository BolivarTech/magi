...
> This is the **pre-MAGI** plan (`planning/claude-plan-tdd-org.md`); after
> Checkpoint 1 (manual review) and Checkpoint 2 (`/magi:magi` gate ≥ GO WITH
> CAVEATS) it is rewritten to `planning/claude-plan-tdd.md`.
...
#### RED (adversarial — nonce non-reuse is MANDATORY)
- [ ] Write the failing test(s)

```rust
#[test]
fn test_encrypt_uses_independent_random_nonce_no_reuse() {
    let vault = CryptoVault::default();
    let password = "same-password";
    let plaintext = "identical plaintext";

    // Two encryptions of the SAME plaintext with the SAME password.
    let blob_a = vault.encrypt(password, plaintext).unwrap();
    let blob_b = vault.encrypt(password, plaintext).unwrap();

    // Ciphertexts must differ (random salt + independent random nonce).
    assert_ne!(blob_a, blob_b, "identical inputs must not produce identical blobs");

    // Decode and recover the embedded nonce from each blob to prove the nonce
    // itself differs (the strong non-reuse guarantee, not just salt variance).
    let nonce_a = extract_nonce_for_test(&blob_a);
    let nonce_b = extract_nonce_for_test(&blob_b);
    assert_ne!(nonce_a, nonce_b, "AES-GCM-SIV nonce must be independent and not reused");
    assert_eq!(nonce_a.len(), 12, "stored nonce is the full AES-GCM-SIV nonce");
}
...
- [ ] Run & verify it fails for the right reason
Run: `cargo nextest run test_encrypt_uses_independent_random_nonce_no_reuse test_new_layout_roundtrips_with_embedded_nonce`
Expected: FAIL — `extract_nonce_for_test` reads `plaindata[SALT_LEN..SALT_LEN+12]`, but the current layout has ciphertext (not a nonce) immediately after the salt, so the extracted "nonce" bytes are actually ciphertext head bytes; more fundamentally the layout has no stored nonce field, so the round-trip helper's assumptions and (once layout changes) the old `decrypt` would mismatch. The non-reuse assertion may pass by accident on salt variance, but `nonce_a.len()==12` against a layout with no nonce slot plus the roundtrip test pin the failure to "no independent nonce stored". Correct Red reason: the independent-nonce layout is unimplemented.
- [ ] Verify rest of §0.1 clean except the intended red tests.
...
```rust
        if plaintext.len() > MAX_PLAINTEXT_LEN {
            return Err(CryptoError::InvalidInput(format!(
                "Plaintext length {} exceeds MAX_PLAINTEXT_LEN ({})",
                plaintext.len(),
                MAX_PLAINTEXT_LEN
            )));
        }
...
```rust
            ...
            // W1: abort instead of growing the SSE buffer without bound.
            if buffer.len() > MAX_SSE_BUFFER_BYTES {
                return stream::iter(vec![Err(anyhow::anyhow!(
                    "SSE buffer exceeded {} bytes without an event boundary; aborting to avoid OOM",
